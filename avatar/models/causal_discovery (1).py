...
from .causal_graph_definition import get_causal_graph
...
class CausalEnhancedDiscovery(nn.Module):
    ...
    def __init__(
        ...
        # 导入预定义因果图
        self.causal_graph = get_causal_graph()
        self.prior_graph = self.causal_graph.get_prior_graph()
        self.prior_weight = prior_weight
        self.node_mapping = self._create_node_mapping()
        self.indexed_prior_graph = self._convert_graph_to_indexed(self.prior_graph, self.node_mapping)
        # 基础组件
        ...
        self.structure_learner = EnhancedStructureLearner(
            ...
            prior_graph=self.indexed_prior_graph,
    ...
    def _convert_graph_to_indexed(self, graph, mapping):
        indexed_graph = nx.DiGraph()
        for edge in graph.edges():
            indexed_graph.add_edge(mapping[edge[0]], mapping[edge[1]])
...
class ClearMechanismPredictor(nn.Module):
    ...
    def forward(self, causal_vars: torch.Tensor, adj_matrix: torch.Tensor) -> torch.Tensor:
        causal_effects = torch.bmm(adj_matrix.unsqueeze(0), causal_vars)
        mechanisms = self.mechanism_predictor(torch.cat([causal_vars, causal_effects], dim=-1))
        return mechanisms
...
class CausalVariableExtractor(nn.Module):
    def __init__(self, input_dim: int, num_vars: int, prior_graph: nx.DiGraph):
        super().__init__()
        # 基础特征提取器
        self.image_projector = nn.Linear(512, 512)  # image_features
        self.text_projector = nn.Linear(512, 512)   # text_features
        
        # 空间和动作特征提取
        self.spatial_extractor = nn.Linear(512, 256)  # spatial_features
        self.action_extractor = nn.Linear(512, 256)   # action_features
        self.entity_loc_extractor = nn.Linear(256, 128)  # entity_location
        self.interaction_extractor = nn.Linear(256, 128)  # interaction_features
        
        # 语义和上下文提取
        self.semantic_extractor = nn.Linear(512, 512)  # semantic_content
        self.context_extractor = nn.Linear(512, 256)   # context

    def forward(self, image_features, text_features):
        # 提取基础特征
        image_vars = self.image_projector(image_features)
        text_vars = self.text_projector(text_features)
        
        # 提取衍生特征
        spatial = self.spatial_extractor(image_vars)
        action = self.action_extractor(image_vars)
        entity_loc = self.entity_loc_extractor(spatial)
        interaction = self.interaction_extractor(action)
        semantic = self.semantic_extractor(torch.cat([image_vars, text_vars], dim=-1))
        context = self.context_extractor(semantic)
        
        return {
            'image_features': image_vars,
